...
class Camera(object):
    ...
    def __init__(self, camidx, robottype='arlo', useCaptureThread = False):
        ...
        if robottype == 'arlo' or robottype == 'frindo':
            ...
            self.intrinsic_matrix = np.asarray([506, 0., 3.0757300398967601e+02, 0.,
                   506, 2.8935674612358201e+02, 0., 0., 1.], dtype = np.float64)
            ...
            self.intrinsic_matrix = np.asarray([9.4328095162920715e+02, 0., 5.1673417797027605e+02, 0.,
                   9.4946668595979077e+02, 3.9546569709311393e+02, 0., 0., 1.], dtype = np.float64)
            self.intrinsic_matrix.shape = (3, 3)
    ...
    def draw_aruco_objects(self, img):
        ...
        if not isinstance(self.ids, type(None)):
            ...
            for i in range(self.ids.shape[0]):
                outimg = cv2.aruco.drawAxis(outimg, self.intrinsic_matrix, self.distortion_coeffs, self.rvecs[i], self.tvecs[i], self.arucoMarkerLength)
        else:
            outimg = img
